...
#define IP_BROADCAST 0xFFFFFF
#define ICMP_TYPE3 0x03
#define ICMP_CODE1 0x01
/*
Based on:
...
void sr_handle_arpreq(struct sr_instance * sr, struct sr_arpreq * request) {
  ...
  /* Lock cache while accessing */
  pthread_mutex_lock(&sr->cache.lock);

  if (difftime(curr_time, request->sent) > 1.0) {
    if (request->times_sent >= 5) {
      fprintf(stderr, "sr_handle_arpreq: arp request sent more than 5 times, sending icmp host unreachable\n");
      struct sr_packet * request_packets = request->packets;
      while (request_packets != NULL) {
        struct sr_if * output_interface = sr_get_interface(sr, request_packets->iface);
        sr_send_icmp_type3(sr, request_packets->buf, request_packets->len, output_interface->name, ICMP_TYPE3, ICMP_CODE1);
    ...
    /*Send arp request*/
      ...
      fprintf(stderr, "sr_handle_arpreq: sending arp request\n");
      /*Find output interface using longest prefix match*/
      while(routing_list != NULL) {
        uint32_t longest_prefix_match = routing_list->mask.s_addr & request->ip;
        if (longest_prefix_match == routing_list->dest.s_addr) {
          fprintf(stderr, "sr_handle_arpreq: calling sr_get_interface on %s\n", routing_list->interface);
          output_interface = sr_get_interface(sr, routing_list->interface);
        }
        routing_list = routing_list->next;
      }
      /*fprintf(stderr, "sr_handle_arpreq: printing output interface\n");
      sr_print_if(output_interface);*/
      ...
      memset(new_packet, 0, PACKET_LENGTH);
      /*Get header pointers*/
      ...
      memset(new_eth_hdr->ether_dhost, IP_BROADCAST, ETHER_ADDR_LEN);
      memcpy(new_eth_hdr->ether_shost, output_interface->addr, ETHER_ADDR_LEN);
      ...
      memcpy(new_arp_hdr->ar_sha, output_interface->addr, ETHER_ADDR_LEN);
      memset(new_arp_hdr->ar_tha, IP_BROADCAST, ETHER_ADDR_LEN);
      new_arp_hdr->ar_sip = output_interface->ip;
      new_arp_hdr->ar_tip = request->ip;

      sr_send_packet(sr, new_packet, PACKET_LENGTH, output_interface->name);
...
void sr_arpcache_sweepreqs(struct sr_instance *sr) {
    ...
    while(request != NULL) {
      struct sr_arpreq * next_ptr = request->next;
